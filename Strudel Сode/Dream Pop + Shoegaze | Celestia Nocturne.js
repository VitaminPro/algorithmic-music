// Celestia Nocturne
// Dream Pop / Shoegaze - Extended Mix
...
stack(
  ...
  stack(
    s("bd:7").gain(0.7).room(0.6).lpf(200),
    s("~ sd:7 ~ sd:7").gain(0.6).room(0.7),
    s("[hh:5 ~ hh:5 ~]*2").gain(0.3).room(0.8).lpf(6000).pan(sine.slow(8))
  ).slow(2).orbit(0),
  ...
  s("wind").gain(0.04).room(0.9).lpf(2000).slow(8).orbit(10),

  // 🌫️ НОВЫЙ: Медленный атмосферный дрон (для длительности)
  note("<e3 e3 c3 d3>")
    .s("triangle")
    .gain(0.2)
    .lpf(500)
  ...
  // 💎 НОВЫЙ: Редкие высокие акценты (появляются раз в 8 тактов)
  note("<~ ~ ~ ~ ~ ~ ~ [b7 e8] ~ ~ ~ ~ ~ ~ ~ [g7 c8]>")
    .s("sine")
    .gain(0.28)
    .room(1)
